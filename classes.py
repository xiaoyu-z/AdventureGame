...
class Player(object):
    ...
    def talk_to(self, person):
        ...
        elif person in [character.name for character in self.place.characters.values()]:
            current_person = self.place.characters[person]
            print(current_person.name,'says:',current_person.talk())
        else:
    ...
    def take(self, thing):
        ...
        elif thing in [thing.name for thing in self.place.things.values()]:
    ...
    def knapsack(self, max_weight, list_of_treasures):
        ...
        27
        """
        list_length = len(list_of_treasures)
        treasure_value_list = [treasure.value for treasure in list_of_treasures]
        treasure_weight_list = [treasure.weight for treasure in list_of_treasures]
        res=[[0]*(max_weight+1) for i in range(list_length+1)]
        for i in range(0,list_length):
            for j in range(max_weight+1):
                res[i][j]=res[i-1][j]
                if j>=treasure_weight_list[i] and res[i-1][j]<res[i-1][j-treasure_weight_list[i]]+treasure_value_list[i]:
                    res[i][j]=res[i-1][j-treasure_weight_list[i]]+treasure_value_list[i]
        return res[list_length-1][max_weight]
        "*** YOUR CODE HERE ***"
